...
'''
def plotNpHist(npHist, ax, col = 'black'):
  print npHist[1]
  ...
  plt.step(npHist[1][1:], y, color=col)
  plt.errorbar(x, y, y_err, x_err, fmt='.', color=col)
'''
...
lines = []
'''
dir_towers = glob.glob("/ta/work/user/hyomin/TAx4_rec/*/")

for dir_tower in dir_towers:
  print "Reading", dir_tower, "\n"
  dir_YYYYMMs = glob.glob(("%s/20*" % dir_tower))
  
  for dir_YYYYMM in dir_YYYYMMs:
    print dir_YYYYMM.split("/")[-1]
    EvtTxts = glob.glob(("%s/EvtTxt/*.txt" % dir_YYYYMM))
    
    for EvtTxt in EvtTxts:
      print EvtTxt.split("/")[-1]
      EvtSummary = open(EvtTxt, 'r').readline()
      #print EvtSummary
      lines.append(EvtSummary)
...
'''
plotNpHist(theta_hist)
plt.show()
plotNpHist(phi_hist)
plt.show()
plotNpHist(energy_gldf_log_hist)
plt.show()
'''
Evts = [\
  yymmdd, \
...
#plt.show()
plt.savefig("%s.EnGT10.pdf" % fHeader)
...
#PO(QC2_Evts, 'energy_ldf', 'GT', 10)
#plt.show()
plt.savefig("%s.EnGT20.pdf" % fHeader)
