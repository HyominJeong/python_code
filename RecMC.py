...
# rusdraw_.yymmdd, rusdraw_.hhmmss, rusdraw_.usec, rufptn_.nstclust, rusdmc_.theta, rusdmc_.phi, rusdmc_.corexyz[0] / 120000, rusdmc_.corexyz[1] / 120000, rusdmc_.corexyz[2] / 120000, rusdmc_.tc, rusdmc_.energy;
...
MCRecResult = open("MCRecResult.rec", 'w')
...
for file in infiles:
  ...
  ycore_MC	= evtInfo[8]
  zcore_MC	= evtInfo[9]
  #t0_MC		= evtInfo[9] # Strange! Need to check!
  energy_MC	= evtInfo[10] # Strange! Need to check!
  ...
  if (float(chi2_geo1) < 1.e6) and (float(chi2_geo2) < 1.e6) and (float(chi2_ldf) < 1.e6) and (float(chi2_gldf) < 1.e6):
    ...
    #geoFit2(xyzclf, reltime, VEM, t0_gldf, xcore_gldf, ycore_gldf, float(theta_gldf), float(phi_gldf), sc_gldf, energy_gldf, chi2_gldf, ndof_gldf, ax3, "Lins. + LDF")
    ...
    #ldfFit(xyzclf, reltime, VEM, t0_gldf, xcore_gldf, ycore_gldf, float(theta_gldf), float(phi_gldf), sc_gldf, energy_gldf, chi2_gldf, ndof_gldf, ax8, "GLDF")
    #plt.show()
  ...
  print>>MCRecResult, \
    ...
    xcore_MC, ycore_MC, zcore_MC, energy_MC, theta_MC, phi_MC
